#!/usr/bin/env python3
"""
Optuna hyperparameter optimization V2 for AlphaZero agent.
Focused optimization based on results from first 42 trials.

Key improvements:
- Fixed parameters that showed low importance
- Narrower ranges for important parameters based on top trials
- More trials to overcome noise and explore better
"""
...
def objective(trial):
    ...
    # ============================================================================
    # FIXED PARAMETERS (based on analysis of first 42 trials)
    # ============================================================================
    
    # MCTS simulations: 200 was good enough, saves compute time
    mcts_simulations = 200
    
    # Policy target method: all top trials used this
    policy_target_method = 'visits_with_selection_bias'
    selection_bias_weight = trial.suggest_float('selection_bias_weight', 0.2, 0.7)
    
    # Depth bonus: keep enabled with narrow range
    use_depth_bonus = True
    depth_bonus = trial.suggest_float('depth_bonus', 0.02, 0.12)
    
    # Virtual loss: top trial used it, keep exploring
    use_virtual_loss = trial.suggest_categorical('use_virtual_loss', [True, False])
    virtual_loss_weight = trial.suggest_float('virtual_loss_weight', 0.3, 0.9) if use_virtual_loss else 0.0
    
    # Critical threshold: narrow range around successful values
    critical_threshold = trial.suggest_float('critical_threshold', 0.96, 0.99)
    
    # ============================================================================
    # IMPORTANT PARAMETERS TO OPTIMIZE (based on top trial patterns)
    # ============================================================================
    
    # Training epochs: CRITICAL - top trials had 4-5, worst had 1-3
    training_epochs = trial.suggest_int('training_epochs', 3, 7)
    
    # Temperature: moderate range, avoid extremes
    # Top trials: 0.59-2.0, worst trials: 0.5 or 2.5+
    temperature = trial.suggest_float('temperature', 0.8, 2.5)
    
    # Temperature decay: keep reasonable range
    temperature_decay = trial.suggest_float('temperature_decay', 0.92, 0.99)
    
    # Dirichlet parameters: encourage exploration to prevent policy collapse
    dirichlet_epsilon = trial.suggest_float('dirichlet_epsilon', 0.25, 0.5)
    dirichlet_alpha = trial.suggest_float('dirichlet_alpha', 0.4, 1.0)
    
    # Learning rate: around the best trial's value
    learning_rate = trial.suggest_float('learning_rate', 3e-5, 3e-4, log=True)
    
    # Replay buffer size: narrower range based on 75 total episodes
    replay_buffer_size = trial.suggest_int('replay_buffer_size', 20, 60, step=20)
    
    # PUCT exploration constant
    puct_c = trial.suggest_float('puct_c', 1.5, 3.5)
    
    # Loss weighting: important for preventing collapse
    # Best trial had policy_weight=1.19, value_weight=2.97
    policy_weight = trial.suggest_float('policy_weight', 0.5, 3.0)
    value_weight = trial.suggest_float('value_weight', 1.5, 3.0)
    
    # Penalty for failure
    penalty_for_failure = trial.suggest_float('penalty_for_failure', -8.0, -5.0)
    
    # Training schedule - fixed for speed
    episodes_per_iteration = 5  # Fixed at 5 episodes per iteration
    
    # Quick training settings for optimization
    num_cycles = 1  # 1 cycle through all training chronics (903 chronics)
    max_training_iterations = 15  # 15 iterations × 5 episodes = 75 total episodes per trial
    ...
    trial_dir = Path(f'optuna_trials_v2/trial_{trial.number}')
    ...
    try:
        ...
        print(f"  FIXED: mcts_simulations={mcts_simulations}, policy_target={policy_target_method}")
        print(f"  training_epochs: {training_epochs}")
        print(f"  temperature: {temperature:.2f}")
        print(f"  learning_rate: {learning_rate:.6f}")
        print(f"  use_virtual_loss: {use_virtual_loss}")
        print(f"{'='*80}\n")
...
def main():
    """Run Optuna optimization V2."""
    
    print("=" * 80)
    print("OPTUNA HYPERPARAMETER OPTIMIZATION V2")
    print("Focused optimization based on first 42 trials")
    print("=" * 80)
    
    # Create study with persistent storage
    storage_path = 'sqlite:///optimization_results_v2/optuna_study_v2.db'
    study = optuna.create_study(
        direction='minimize',  # Minimize negative avg steps = maximize avg steps
        study_name='alphazero_optimization_v2',
        storage=storage_path,  # Persistent SQLite storage - can resume later
        load_if_exists=True,  # Resume if study already exists
        sampler=optuna.samplers.TPESampler(seed=43),  # Tree-structured Parzen Estimator with new seed
    ...
    # Run optimization
    n_trials = 75  # More trials to overcome noise with fewer hyperparameters
    
    print(f"\nRunning {n_trials} optimization trials...")
    print("This will take a while. Each trial trains for 15 iterations with 5 episodes each.\n")
    print("Key changes from V1:")
    print("  - Fixed mcts_simulations=200 (saves time)")
    print("  - Narrower ranges for important params (training_epochs, temperature, etc.)")
    print("  - More trials (100 vs 42) to overcome noise")
    print("  - 75 total episodes per trial (15 iter × 5 episodes)")
    print("  - Focused on preventing policy collapse\n")
    ...
    # Save results
    results_dir = Path('optimization_results_v2')
    results_dir.mkdir(exist_ok=True)
    
    # Save best parameters to JSON
    best_params_path = results_dir / 'best_hyperparameters_v2.json'
    with open(best_params_path, 'w') as f:
        json.dump(study.best_trial.params, f, indent=2)
    print(f"\n✅ Best parameters saved to: {best_params_path}")
    
    # Save optimization history
    history_path = results_dir / 'optimization_history_v2.csv'
    ...
    if len(study.trials) > 1:
        try:
            ...
            fig_importance.write_html(str(results_dir / 'param_importance_v2.html'))
            
            # Optimization history
            fig_history = optuna.visualization.plot_optimization_history(study)
            fig_history.write_html(str(results_dir / 'optimization_history_v2.html'))
            
            # Parallel coordinate plot
            fig_parallel = optuna.visualization.plot_parallel_coordinate(study)
            fig_parallel.write_html(str(results_dir / 'parallel_coordinate_v2.html'))
