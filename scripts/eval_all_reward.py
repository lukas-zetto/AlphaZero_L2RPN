#!/usr/bin/env python3
"""
Multi-run evaluation for Reward Function Experiments  
...
def create_reward_plots_with_confidence_bands(all_results, output_dir):
    """Create plots with smooth interpolated confidence bands across runs"""
    
    print(f"\n📊 Creating reward function comparison plots with smooth confidence bands...")
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Reward function variants (only keep ones that actually exist in results)
    preferred_variants = ["AlphaZero", "D3QN-2022", "D3QN-2020", "Loss", "MaxRho", "PPO", "LinesCapacity"]
    variants = [
        v for v in preferred_variants
        if any(v in run_data and run_data[v] for run_data in all_results.values())
    ]
    if not variants:
        print("  ⚠️  No reward variants with data found!")
        return
    ...
    # Filter data to only include common runs
    filtered_variant_run_data = {}
    for variant in variants:
        filtered_variant_run_data[variant] = {}
        for run_suffix, run_points in variant_run_data.get(variant, {}).items():
            if run_suffix in common_runs:
                filtered_variant_run_data[variant][run_suffix] = run_points
                print(f"    {variant} run {run_suffix}: {len(run_points)} data points after 40M filter")
    
    variant_run_data = filtered_variant_run_data
    ...
    # Limit to 50M steps to ensure fair comparison across all runs
    x_max = min(x_max, 50.0)  # 50M steps
    ...
    axes[0].set_title('Reward Function Experiments - Average Reward', fontsize=24, fontweight='bold')
    ...
    axes[1].set_title('Reward Function Experiments - Average Steps Survived', fontsize=24, fontweight='bold')
    for i, variant in enumerate(variants):
        if variant in interpolated_data:
            data = interpolated_data[variant]
    ...
    axes[2].set_title('Reward Function Experiments - Survival Rate', fontsize=24, fontweight='bold')
    for i, variant in enumerate(variants):
        if variant in interpolated_data:
            data = interpolated_data[variant]
    ...
    pdf_filename = output_dir / f"reward_function_smooth_confidence_bands_{timestamp}.pdf"
    plt.savefig(pdf_filename, bbox_inches='tight')
    print(f"  📈 Saved plot: {pdf_filename}")
    
    # Also save as PNG
    png_filename = output_dir / f"reward_function_smooth_confidence_bands_{timestamp}.png"
...
def create_reward_steps_only_plot(all_results, output_dir):
    """Create a plot showing only average steps survived comparison for reward functions"""
    
    print("📊 Creating reward steps-only plot...")
    
    # Setup - include only variants that actually exist in results
    preferred_variants = ["AlphaZero", "Loss", "MaxRho", "D3QN-2020", "D3QN-2022", "PPO", "LinesCapacity"]
    variants = [
        v for v in preferred_variants
        if any(v in run_data and run_data[v] for run_data in all_results.values())
    ]
    ...
    colors = {'AlphaZero': '#00838F', 'D3QN-2020': '#C62828', 'D3QN-2022': '#1565C0', 'Loss': '#F9A825', 'MaxRho': '#6A1B9A', 'PPO': '#2E7D32', 'LinesCapacity': '#888888'}
    ...
    for variant in variants:
        if variant in variant_run_data and variant_run_data[variant]:
            ...
            if all_steps:
                ...
                # Plot
                ax.plot(x_common[steps_valid], mean_steps[steps_valid], color=colors.get(variant, '#000000'), linewidth=3, label=f'{variant}')
                ax.fill_between(x_common[steps_valid], mean_steps[steps_valid] - std_steps[steps_valid], mean_steps[steps_valid] + std_steps[steps_valid], 
                              color=colors.get(variant, '#000000'), alpha=0.2)
    
    ax.set_xlabel('Training Steps (Millions)', fontsize=22)
    ax.set_ylabel('Average Steps Survived', fontsize=22)
    ax.set_title('Reward Function Comparison: Average Steps Survived', fontsize=24, fontweight='bold')
    ...
    pdf_filename = output_dir / f"reward_steps_only_{timestamp}.pdf"
    plt.savefig(pdf_filename, format='pdf', bbox_inches='tight')
    print(f"  📈 Saved steps plot: {pdf_filename}")
    
    png_filename = output_dir / f"reward_steps_only_{timestamp}.png"
...
def evaluate_experiments(): 
    """Multi-run evaluation for reward function experiments with organized subfolders"""
    
    print("🚀 Multi-run Reward Function Evaluation with Confidence Bands")
    print("=" * 70)
    ...
    results_dir = Path(eval_base) / "reward_multi_run_results"
    results_dir.mkdir(parents=True, exist_ok=True)
    
    # Runs to evaluate
    runs = ["_5", "_6", "_7", "_8"]
    
    # Reward function experiments structure
    experiments = [
        {"name": "AlphaZero", "subdir": "checkpoints_reward_alphazero"},
        {"name": "D3QN-2022", "subdir": "checkpoints_reward_d3qn2022"},
        {"name": "D3QN-2020", "subdir": "checkpoints_reward_d3qn2020"},
        {"name": "Loss", "subdir": "checkpoints_reward_loss"},
        {"name": "MaxRho", "subdir": "checkpoints_reward_maxrho"},
        {"name": "PPO", "subdir": "checkpoints_reward_ppo"},
        {"name": "LinesCapacity", "subdir": "checkpoints_reward_linescapacity"}
    ]
    ...
    for run_suffix in runs:
        ...
        experiment_name = f"reward_experimentsfixed{run_suffix}"
    ...
    json_file = results_dir / "reward_multi_run_results.json"
    ...
    if all_results:
        ...
        for run_suffix, run_data in all_results.items():
            for exp_name, exp_results in run_data.items():
                ...
                subdir_mapping = {"AlphaZero": "checkpoints_reward_alphazero", "D3QN-2020": "checkpoints_reward_d3qn2020", "D3QN-2022": "checkpoints_reward_d3qn2022", "Loss": "checkpoints_reward_loss", "MaxRho": "checkpoints_reward_maxrho", "PPO": "checkpoints_reward_ppo"}
    ...
    if all_results:
        plot_data = create_reward_plots_with_confidence_bands(all_results, results_dir)
    ...
    print(f"\n🎉 Reward function evaluation complete!")
...
def main(plot_only=False, results_file=None):
    """Multi-run evaluation for reward experiments with organized subfolders"""
    ...
    if plot_only:
        ...
        if all_results:
            ...
            create_reward_steps_only_plot(all_results, results_dir)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Multi-run Reward Experiment Evaluation')
