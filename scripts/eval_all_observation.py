#!/usr/bin/env python3
"""
Multi-run evaluation for Observation Space Experiments  
...
def get_available_cpus():
    ...
    return 1  # Safe default for testing; increase for production runs
...
def create_observation_plots_with_confidence_bands(all_results, output_dir):
    """Create plots with smooth interpolated confidence bands across runs"""
    
    print(f"\n📊 Creating observation space comparison plots with smooth confidence bands...")
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Observation space variants
    variants = ["Minimal", "Essential", "Custom", "Gym"]
    colors = {'Minimal': '#00838F', 'Essential': '#1565C0', 'Custom': '#F9A825', 'Gym': '#6A1B9A'}
    ...
    def summarize_curves(curves):
        ...
        min_runs = min(2, len(curves))
        valid_mask = n_valid >= min_runs
    ...
    axes[0].set_title('Observation Space Experiments - Average Reward', fontsize=24, fontweight='bold')
    for variant in variants:
    ...
    axes[1].set_title('Observation Space Experiments - Average Steps Survived', fontsize=24, fontweight='bold')
    for variant in variants:
        if variant in interpolated_data:
            data = interpolated_data[variant]
    ...
    axes[2].set_title('Observation Space Experiments - Survival Rate', fontsize=24, fontweight='bold')
    for variant in variants:
        if variant in interpolated_data:
            data = interpolated_data[variant]
    ...
    pdf_filename = output_dir / f"observation_space_smooth_confidence_bands_{timestamp}.pdf"
    plt.savefig(pdf_filename, bbox_inches='tight')
    print(f"  📈 Saved plot: {pdf_filename}")
    
    # Also save as PNG
    png_filename = output_dir / f"observation_space_smooth_confidence_bands_{timestamp}.png"
...
def create_observation_steps_only_plot(all_results, output_dir):
    """Create a plot showing only average steps survived comparison for observation spaces"""
    
    print("📊 Creating observation steps-only plot...")
    ...
    for variant in variants:
        if variant in variant_run_data and variant_run_data[variant]:
            ...
            if all_steps:
                ...
                # Plot
                ax.plot(x_common[steps_valid], mean_steps[steps_valid], color=colors[variant], linewidth=3, label=f'{variant}')
    ...
    ax.set_title('Observation Space Comparison: Average Steps Survived', fontsize=24, fontweight='bold')
    ...
    pdf_filename = output_dir / f"observation_steps_only_{timestamp}.pdf"
    plt.savefig(pdf_filename, format='pdf', bbox_inches='tight')
    print(f"  📈 Saved steps plot: {pdf_filename}")
    
    png_filename = output_dir / f"observation_steps_only_{timestamp}.png"
...
def main(plot_only=False, results_file=None): 
    """Multi-run evaluation for observation space experiments with organized subfolders"""
    
    # Handle plot-only mode first
    if plot_only:
        ...
        # Create plots
        if all_results:
            plot_data = create_observation_plots_with_confidence_bands(all_results, results_dir)
            
            # Create separate steps-only plot
            create_observation_steps_only_plot(all_results, results_dir)
    ...
    print("🚀 Multi-run Observation Space Evaluation with Confidence Bands")
    print("=" * 70)
    ...
    results_dir = Path(eval_base) / "observation_multi_run_results"
    results_dir.mkdir(parents=True, exist_ok=True)
    
    # Runs to evaluate
    runs = ["_5", "_6", "_7", "_8"]
    
    # Observation space experiments structure
    experiments = [
        {"name": "Minimal", "subdir": "checkpoints_obs_minimal", "obs_space_type": "minimal"},
        {"name": "Essential", "subdir": "checkpoints_obs_essential", "obs_space_type": "essential"},
        {"name": "Custom", "subdir": "checkpoints_obs_custom", "obs_space_type": "custom"},
        {"name": "Gym", "subdir": "checkpoints_obs_gym", "obs_space_type": "gym"}
    ]
            ...
            if not checkpoints_to_eval:
                ...
                with ProcessPoolExecutor(max_workers=max_workers) as executor:
                    future_to_checkpoint = {
                        executor.submit(evaluate_checkpoint, cp, f"{experiment_name}_{exp['name']}", exp['obs_space_type']): cp 
    ...
    json_file = results_dir / "observation_multi_run_results.json"
    ...
    if all_results:
        ...
        for run_suffix, run_data in all_results.items():
            for exp_name, exp_results in run_data.items():
                ...
                subdir_mapping = {"Minimal": "checkpoints_obs_minimal", "Custom": "checkpoints_obs_custom"}
    ...
    print(f"\n🎉 Observation space evaluation complete!")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Multi-run Observation Space Experiment Evaluation')
